...
async function fixUsersRLSV3() {
  ...
  try {
    ...
    `);
    console.log('   Available roles:', enumCheck.rows.map(r => r.enumlabel).join(', '));
    console.log('');
    ...
    // Policy 3: Admins can SELECT all users (only checking for 'admin' role)
